...
public class JTetris extends JComponent {
    ...
    public Piece[] pieceArray = PIECES;
    int counter = 0;
    JTetris() {
        super();
        ...
        timer = new javax.swing.Timer(DELAY, new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                /*newBrain bestBrain = new newBrain();
                for(int i = 0; i < 5; i++){
                    if(board.getCurrentPiece() == null || board.testMove(Board.Action.DROP) == null){
                        break;
                    }
                    Board.Action nextMove = bestBrain.nextMove(board);
                    //System.out.println("Automoving");
                    tick(nextMove);
                }*/
                tick(Board.Action.DOWN);
            }
    ...
    public Piece pickNextPiece() {
        if(counter%7==0)
        {
            ...
            {
                int toShift = random.nextInt(i+1);
                Piece temp = pieceArray[i];
                pieceArray[i]=pieceArray[toShift];
                pieceArray[toShift]=temp;
            }
        }
        counter++;
        //return new TetrisPiece(Piece.PieceType.STICK);
        return pieceArray[(counter-1)%7];
    }
    ...
    public void tick(Board.Action verb) {
        ...
        if(currPiece != null){
            ...
            System.out.println(board.getLastAction() + " " + board.getLastResult() + " " + tick++);
        }
        repaint();
    }
